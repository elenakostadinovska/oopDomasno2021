import java.io.File;
import javax.xml.parsers.DocumentBuilder;
import javax.xml.parsers.DocumentBuilderFactory;
import javax.xml.transform.Transformer;
import javax.xml.transform.TransformerFactory;
import javax.xml.transform.dom.DOMSource;
import javax.xml.transform.stream.StreamResult;
import org.w3c.dom.Document;
import org.w3c.dom.Element;
public class XmlCreator {
    public void createXMLFile(String fileName, Student vraboten) {
        try {
//Креирање на документ
            DocumentBuilderFactory documentBuilderFactory =

                    DocumentBuilderFactory.newInstance();

            DocumentBuilder documentBuilder =

                    documentBuilderFactory.newDocumentBuilder();

            Document document = documentBuilder.newDocument();
//Креирање на елементите на документот
//Креирање на root елементот
            Element root = document.createElement("Student");
            document.appendChild(root);
//Креирање на елементот - Име
            Element ime = document.createElement("Ime");
            root.appendChild(ime);

//Креирање на елементот - Презиме
            Element prezime = document.createElement("Prezime");
            root.appendChild(prezime);
//Креирање на елементот - Плата
            Element plata = document.createElement("Index");
            root.appendChild(plata);
//Доделување вредности
            ime.appendChild(document.createTextNode(vraboten.getIme()));
            prezime.appendChild(document.createTextNode(vraboten.getPrezime()));
            plata.appendChild(document.createTextNode(vraboten.getIndex()));




            TransformerFactory transformerFactory =

                    TransformerFactory.newInstance();

            Transformer transformer =

                    transformerFactory.newTransformer();

            DOMSource source = new DOMSource(document);
//Фајл
            StreamResult result = new StreamResult(new File(fileName));
//Конзола
            StreamResult result1 = new StreamResult(System.out);
            transformer.transform(source, result);
            System.out.println("Fajlot e kreiran, imeto e: "+

                    fileName);

            System.out.println(result1);
            System.out.println("Fajlot e zachuvan.");
        } catch (Exception e) {
            System.out.println(e.toString());
        }
    }
}

